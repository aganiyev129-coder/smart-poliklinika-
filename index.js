...
app.get('/api/portal/appointments', (req, res) => {
  ...
  if (patientId) res.json(db.prepare("SELECT * FROM appointments WHERE patientId=? ORDER BY date DESC, time ASC").all(patientId));
  else if (patientName) res.json(db.prepare("SELECT * FROM appointments WHERE patientName=? ORDER BY date DESC, time ASC").all(patientName));
  else res.json([]);
});
app.post('/api/portal/book', (req, res) => {
  ...
  const r = db.prepare('INSERT INTO appointments (patientId,patientName,doctorId,doctorName,date,time,queue,price,note) VALUES (?,?,?,?,?,?,?,?,?)').run(patientId || null, patientName || 'Mehmon', doctorId, doctorName, date, time, sameDay + 1, price || 0, 'Portal orqali belgilangan');
...
io.on('connection', (socket) => {
  ...
  socket.on('send-message', ({ chatKey, message }) => {
    db.prepare('INSERT INTO chats (chatKey,text,sender,isPrescription,time) VALUES (?,?,?,?,?)').run(chatKey, message.text, message.sender, message.isPrescription ? 1 : 0, new Date().toISOString());
